...
export const Map = React.forwardRef((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        ...
        drawDestinationMarkers: (coords) => {
            ...
            var markers = coords.map((x, idx) => {
                const markerName = idx === 0 || (idx + 1 === coords.length) ? 'D' : idx + 1;
                return new window.google.maps.Marker({
                map: mapInstanceRef.current,
                position: x,
                icon: `https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=${markerName}|6750A4|000000`
            })});
