    ...
    return (<View style={styles.container}>
        ...
        <ScrollView style={styles.scrollView}>
            <List.Item
                title="Active routes"
            />
            {dates.length > 0 && dates[0] !== undefined ? dates.map((date, index) => (<List.Item
                key={index}
                title={date}
...
const styles = StyleSheet.create({
    container: {
        ...
        marginLeft: 16, marginTop: 40, flexDirection: 'column',
    }, user: {
        marginLeft: 16, marginTop: 8, flexDirection: 'column',
    }, scrollView: {
